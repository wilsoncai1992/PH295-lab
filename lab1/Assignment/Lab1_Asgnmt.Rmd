...
- If the kustosis of the residuals is $\le 3$, fit a linear regression with log 
link: 
$$ E(Y | A,W) = exp(\alpha_0 + \alpha_1 A + \alpha_2 W ) \ . $$

- Compute the residual from this regression 
$$ \varepsilon_{2,i} = Y_i - \hat{Y}_i = Y_i - exp(\alpha_{0,n} + \alpha_{1,n} A_i + \alpha_{2,n} W_i) $$

- Using this residual, fit the following linear regression model with log-link:
$$ E(\varepsilon_{2,i}^2 | \hat{Y})= exp\{\lambda_0 + \lambda_1 log(\hat{Y})\} $$
That is, using a normal GLM with log link, regress the squared residuals from the raw-scale regression on the log-transformed predicted values from the raw-scale regression. 
